        ...
        <div class="content">
            <div class="row">
                <table class="table-userlist">
                    ...
                    <c:forEach items="${games}" var="news">
                        <tr>
                            <td>${news.id}</td>
                            <td>${news.gameTitle}</td>
                            <td>${news.reviewAuthor}</td>
                            <td>${news.rating}</td>
                            <td>${news.reviewDate}</td>
                            <td>${news.reviewTime}</td>
                            <td>
                                <div class="game-list"><img src="/resources/assets/img/post/${news.file}"></div>
                            </td>
                            <td><a href='<c:url value="/editGame/${news.id}"/>'>
                                <button type="submit" class="btn btn-info">Edytuj</button>
                            </a></td>
                            <td>
                                <form class="form" method="post" action='<c:url value="/deleteGame/${news.id}"/>'>
                                    <button class="btn-right btn btn-danger" role="button"
                                            onclick="if (!(confirm('WARNING!! After submitting, all comments to this' +
                                             ' current game post will be permamently DELETED. Are you sure you want to proceed?')))
                                                return false">Usuń
                                    </button>
                                </form>
                            </td>
